...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        //load texture cords
        if ( mesh->mTextureCoords[0] ) // check if texture coords are available
        {
            glm::vec2 vec;
    ...
    // 3. normal maps
    std::vector<Textures> normalMaps = loadMaterialTextures(material, aiTextureType_NORMALS, "texture_normal");
